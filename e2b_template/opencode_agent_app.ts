 ...
 * ==========================
 *
 * OpenCode SDK-based orchestrator for running GLM-4.7 agents in E2B sandboxes.
...
// Project directory (from environment or default for E2B)
const PROJECT_DIR = process.env.PROJECT_DIR || "/home/user/project";

// Host API configuration (for graceful stop checks)
...
// Agent log file
const AGENT_LOG_FILE = process.env.AGENT_LOG_FILE || "/var/log/agent.log";
 ...
 */
// Track API failures to avoid log spam
let apiFailureLogged = false;

/**
 * Check if graceful stop was requested via file or API
 */
function checkGracefulStop(): boolean {
  // Check file first (backwards compatibility)
  try {
  ...
  try {
    // Use synchronous HTTP request (simple approach for TypeScript)
    const http = require("http");
    const url = `${HOST_API_URL}/api/projects/${PROJECT_NAME}/agent/containers/${CONTAINER_NUMBER}/session`;

    // Note: This is a simplified sync check - in production you might want async
    // For now, we rely primarily on the file-based check
    return false;
  } catch (e: any) {
    if (!apiFailureLogged) {
      log("WARN", `Failed to check graceful stop via API: ${e.message}`);
