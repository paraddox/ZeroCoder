...
- Loads environment variables from `.env` file (API keys like `ANTHROPIC_API_KEY`, `E2B_API_KEY`)
- Passes these to E2B sandboxes so agents can authenticate
- Handles graceful shutdown of sandboxes on Ctrl+C
...
### E2B Sandboxes (Per-Project Cloud Execution)

The system uses E2B cloud sandboxes for isolated agent execution:

```bash
# Build and deploy the E2B template (one-time setup)
cd e2b_template
e2b template create
...
HOST_API_URL=https://...          # Public URL for host API (sandbox → host)
SSH_PRIVATE_KEY_BASE64=...        # Base64-encoded SSH key for git
```

**Architecture:**
- Host runs FastAPI server + React UI (project management, progress monitoring)
- Each project gets its own E2B sandbox with Claude Agent SDK + beads CLI
- Sandboxes clone the repo at startup from `GIT_REMOTE_URL`
- SSH key passed via environment variable (base64 encoded), decoded at sandbox startup
- Multiple sandboxes can run simultaneously for different projects
- 60-second staggered startup between sandboxes to allow git clone

**Sandbox lifecycle:**
- `not_created` → `running` → `stopped` (5 min idle timeout) → `completed`
- Stopped sandboxes can restart with fresh environment
- Progress visible via cached data (polled from sandbox every 30s)
...
- Auto-restarts crashed agents and stopped sandboxes (user-started only)
- Skips sandboxes already in restart process

**Sandbox naming:** `zerocoder-{project-name}-{N}` (e.g., `zerocoder-nexus-1`, `zerocoder-nexus-2`)
...
- Sandboxes are fully standalone - they clone repos at runtime
- SSH key passed via environment variable (base64 encoded)
- No volume mounts required - sandboxes are fully isolated

**Key services:**
- `server/services/e2b_sandbox_manager.py` - E2B sandbox lifecycle and agent control
- `server/services/local_project_manager.py` - Local clones for wizard/edit mode
- `registry.py` - Project and sandbox metadata
...
- `server/routers/features.py` - Feature management via sandbox commands
- `server/routers/agent.py` - Sandbox control (start/stop/remove)
- `server/routers/spec_creation.py` - WebSocket for interactive spec creation
- `server/services/e2b_sandbox_manager.py` - Per-project E2B sandbox lifecycle
- `server/services/sandbox_beads.py` - Send beads commands to sandboxes via E2B commands API
...
- All beads operations route through E2B sandbox commands API
- `e2b_template/beads_commands.py` - Handles CRUD operations inside sandbox
- Feature data cached in SQLite (`FeatureCache`, `FeatureStatsCache` models)
- Background poller updates cache every 30 seconds when sandbox running
- Write operations (create, update, delete, reopen) require sandbox to be running
...
Defense-in-depth approach using E2B cloud sandboxes:
1. Each project runs in isolated E2B sandbox (no local filesystem access)
2. Sandbox clones repo fresh at startup - fully isolated environment
3. SSH key passed via environment variable (base64 encoded)
4. Claude credentials passed via environment variables
5. E2B manages infrastructure isolation and security
...
1. Start project sandbox via UI (creates E2B sandbox `zerocoder-{project}-N`)
2. Sandbox runs Claude Agent SDK with project-specific `CLAUDE.md`
...
- Runs in sandbox 0 (`zerocoder-{project}-0`), separate from coding sandboxes
...
- `log` - Agent output lines (streamed from sandbox stdout/stderr)
